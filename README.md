...
| 機能 | 状態 | 備考 |
|------|------|------|
| WebSocket サーバー (port 19800) | **動作確認済** | WSA 互換プロトコル、CORS 制御あり |
...
| code | 意味 |
|------|------|
| `UNSTABLE` | 計量値が安定しない（最大リトライ超過） |
| `TIMEOUT` | スケールからの応答タイムアウト（3秒） |
| `OVERLOAD` | 計量器の過負荷 |
| `PORT_ERROR` | シリアルポートエラー・未接続 |
| `INVALID_REQUEST` | JSON パースエラー |
