...
ラズパイへの配備・OS 設定・サービス運用の正本は [raku-sika-ops](https://github.com/deervery/raku-sika-ops)。

- 初期セットアップ: `ops/setup/raspi-setup.sh`
- 標準再デプロイ: `raku-sika-redeploy-hub-clean`
- 構成差分監査: `raku-sika-drift-audit`

Hub README は「アプリ仕様と API」、Ops README は「端末運用手順」を扱う。
...
| GET / DELETE | `/printer/queue` | CUPS キュー確認 / 全削除 |
...
- `/health` の `printer.connected` は CUPS 上でプリンタ名を解決できることを示す
